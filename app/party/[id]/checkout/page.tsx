...
export default function CheckoutPage() {
  ...
  function validate(): string | null {
    ...
    if (quantity > maxLimit)
      return `Max ${maxLimit} tickets per order for this tier`;
    if (quantity < 1 || quantity > available)
      return `Only ${available} tickets available`;

    const currency = party?.currency_code ?? "NGN";
    if (!PAYSTACK_CURRENCIES.includes(currency))
      return `Currency ${currency} not supported by Paystack`;
    return null;
  }
  ...
  async function handlePaymentSuccess(reference: string) {
    ...
    try {
      ...
      if (fnError || !result?.success) {
        throw new Error(
          fnError?.message ?? result?.error ?? "Ticket creation failed",
        );
      }
      ...
      if (p.total === 0) {
        setError(
          `Ticket creation failed: ${actualError}. Please try again. (Ref: ${reference})`,
        );
      } else {
  ...
  const BankTransferModal = () => (
    ...
    >
      ...
      >
        ...
        >
          Bank transfers take a few minutes to confirm. Your ticket will be{" "}
          <strong style={{ color: "#a855f7" }}>
            automatically emailed to you within 5–10 minutes
          </strong>{" "}
  ...
  return (
    <>
      ...
      <div
        style={{ maxWidth: 560, margin: "0 auto", padding: "120px 24px 100px" }}
      >
        <h1
          style={{
            margin: "0 0 8px",
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div>
              ...
              >
                Email (ticket will be sent here)
              </label>
              <input
                type="email"
                value={guestEmail}
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              >
                ...
                <span
                  style={{ color: "#a855f7", fontSize: 18, fontWeight: 800 }}
                >
                  {total === 0
                    ? "Free"
                    : `${symbol}${total.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 })}`}
                </span>
              </div>
            </div>
          </div>
        ...
        <button
          ...
          style={{
            width: "100%",
            background:
              purchasing || !paystackReady || !selectedTier
        ...
        >
          {purchasing ? (
            ...
            `Pay ${symbol}${total.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 })} →`
          )}
        </button>
